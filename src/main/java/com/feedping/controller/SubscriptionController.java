...
import com.feedping.dto.request.GetSubscriptionListRequest;
...
public class SubscriptionController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<Void>> subscribeRss(@RequestBody @Valid RssSubscriptionRequest request) {
    ...
    public ResponseEntity<ApiResponse<RssSubscriptionPageResponse>> getSubscriptionManagementPage(
            @RequestBody @Valid GetSubscriptionListRequest request,
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable
    ) {
        RssSubscriptionPageResponse page = subscriptionService.getSubscriptionsByEmail(request.getToken(), pageable);
